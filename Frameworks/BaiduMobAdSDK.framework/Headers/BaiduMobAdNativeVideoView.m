...
@property (nonatomic, strong) UIImageView *endcardview;//视频后贴图
@property (nonatomic, strong) NSString *endcardUrlString;//视频后贴url
...
- (instancetype)initWithFrame:(CGRect)frame andObject:(BaiduMobAdNativeAdObject *)object {
    if (self = [super initWithFrame:frame]) {
        ...
        _isAutoPlay = YES;
        _supportActImage = NO;
        _supportControllerView = NO;
        _endcardUrlString = object.mainImageURLString;
    }
    return self;
}
...
- (void)setupUI {
    ...
    if (!self.endcardview) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
            NSData *imageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:self.endcardUrlString]];
            dispatch_async(dispatch_get_main_queue(), ^{
                imageView.image = [UIImage imageWithData:imageData];
            });
        });
        imageView.frame = self.displayView.bounds;
        imageView.userInteractionEnabled = YES;
        [self.displayView addSubview:imageView];
        self.endcardview = imageView;
        float x = self.displayView.bounds.size.width/2;
        float y = self.displayView.bounds.size.height/2;
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(x - 30, y - 30, 60, 60)];
        [btn setImage:[self imageResoureForName:@"player_play"] forState:UIControlStateNormal];
...
- (void)playClick:(UIButton *)sender {
    if (!self.endcardview || self.endcardview.hidden) {
        if(self.player.rate==0){ //当前暂停
